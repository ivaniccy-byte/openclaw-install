...
- **项目名称**: OpenClaw 一键安装版（别名：ClawWork）
- **产品定位**: OpenClaw 3.28原版、无捆绑、纯原版内核、全程零代码、一键安装的OpenClaw整合包
- **目标用户**: 国内零技术基础普通上班族
...
- **存储**: SSD 固态硬盘，系统盘(C:)可用空间 ≥15GB (V1.0 强制要求)
- **磁盘空间**: ≥10GB (SSD 推荐)
...
### 4. CI/CD 云编译无产物下载
- **方案**: 修复 Actions 工作流，增加 `upload-artifact` 步骤，消除所有 Node.js 20 Action 废弃告警与 macOS Node 版本链接冲突。

### 5. 前端UI与后端状态失步
- **方案**: 修复「功能管理」页面的记忆系统选择与配置中心联动，完善大模型 API 连通性测试（补全/chat/completions路径处理），增加一键申请 API 密钥入口。

### 6. 环境检测阈值过严 (4GB内存拦截)
- **方案**: 将内存检测阈值下调至 3.5GB，磁盘下调至 9.5GB，并改用浮点数计算，解决因硬件保留内存（如集成显卡）导致 4GB 物理内存系统被误判为不足的问题。
...
| 版本 | 日期 | 说明 |
|------|------|------|
| v0.9.3 | 2026-04-11 | **修复硬件检测与CI优化**: 解决4GB内存误报问题，修复产物丢失并优化模型管理联动 |
